...
class Voxels(object):
    ...
    def get_index_for_position(self, position: np.ndarray) -> np.ndarray:
        """Get the voxel index for a given position

        Args:
            position (np.array): _description_

        Returns:
            np.array: _description_
        """
        x_n = (position[0] - self.translate[0]) / self.scale
        y_n = (position[1] - self.translate[1]) / self.scale
        z_n = (position[2] - self.translate[2]) / self.scale
        i = int(np.around(x_n * self.dims[0] - 0.5))
        j = int(np.around(y_n * self.dims[1] - 0.5))
        k = int(np.around(z_n * self.dims[2] - 0.5))
        return np.array([i, j, k])

    def get_position_for_index(self, i: int, j: int, k: int) -> Tuple[float, float, float]:
        """numpy implementation of the following code given by the class description
        input is an array of shape (n, 3)
        x_n = (i+.5)/self.dims[0]
        y_n = (j+.5)/self.dims[1]
        z_n = (k+.5)/self.dims[2]
        x = scale*x_n + self.translate[0]
        y = scale*y_n + self.translate[1]
        z = scale*z_n + self.translate[2]
        """
        p_i = np.divide(np.add(i, 0.5), self.dims[0])
        p_j = np.divide(np.add(j, 0.5), self.dims[1])
        p_k = np.divide(np.add(k, 0.5), self.dims[2])
        p_x = np.add(np.multiply(self.scale, p_i), self.translate[0])
        p_y = np.add(np.multiply(self.scale, p_j), self.translate[1])
        p_z = np.add(np.multiply(self.scale, p_k), self.translate[2])
        return p_x, p_y, p_z
...
def read_header(fp):
    ...
    dims = np.array(list(map(int, fp.readline().strip().split(b" ")[1:])))
    translate = np.array(list(map(float, fp.readline().strip().split(b" ")[1:])))
    scale = np.array(list(map(float, fp.readline().strip().split(b" ")[1:]))[0])
    line = fp.readline()
...
def read_as_3d_array(fp, fix_coords=True):
    ...
    # raw_data = np.frombuffer(fp.read(), dtype=np.uint16)
